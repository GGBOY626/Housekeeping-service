...
import { getActivePath } from '@/router'
...
// 选择
const active = computed(() => getActivePath(route))
// 菜单列表
const list = computed(() => {
  const { navData } = props
...
/** 菜单点击：去掉 :to，仅由此处统一跳转，避免 t-menu 内部拦截导致不发起请求 */
...
// 跳转到评价系统
const handleToReply = async () => {
  const baseUrl = evaluationBaseUrl
