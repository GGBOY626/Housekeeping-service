...
import { ref, onMounted, onActivated, watchEffect, watch } from 'vue'
...
onMounted(() => {
  console.log('[serviceType] onMounted - 组件挂载，发送请求')
  fetchData(requestData.value)
})

onActivated(() => {
  console.log('[serviceType] onActivated - 从 keep-alive 缓存激活')
  // 从缓存激活时，如需刷新列表可取消下行注释
  // fetchData(requestData.value)
})
</script>
