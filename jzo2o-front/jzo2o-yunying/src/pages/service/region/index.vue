<!-- 区域管理 -->
<template>
  <router-view v-if="route.path !== '/service/region'"></router-view>
...
import { ref, watchEffect, onActivated, onMounted, onBeforeMount } from 'vue'
...
const loadPageData = (resetPage = false) => {
  url.value = route.path
  if (route.path !== '/service/region') return
...
onMounted(() => {
  console.log('[region] onMounted - 组件挂载，发送请求')
  loadPageData(true)
})

onActivated(() => {
  console.log('[region] onActivated - 从 keep-alive 缓存激活')
  loadPageData(false)
})
