...
// 获取数据（订单统计依赖 jzo2o-orders-history 服务，该服务不可用时仅提示，不抛错保证页面可继续使用）
const fetchData = async (val) => {
  if (val?.length >= 2 && val[0].length <= 10) {
  ...
  try {
    ...
    const msg =
      status === 503 || err?.message?.includes('503')
        ? '订单统计服务(jzo2o-orders-history)暂不可用，请启动该服务后重试；其他功能可正常使用'
        : err?.message || err?.msg || '获取首页数据失败'
    try {
