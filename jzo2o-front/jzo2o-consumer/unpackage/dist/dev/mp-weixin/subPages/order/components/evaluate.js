  ...
  setup(__props) {
    ...
    const getNewData = async (type) => {
      ...
      await pages_api_order.getOrderScroll(params).then((res) => {
        ...
        common_vendor.index.__f__("log", "at subPages/order/components/evaluate.vue:337", err);
    ...
    const uploadImage = (result) => {
      ...
      for (let i = 0; i < tempFilePaths.length; i++) {
        const uploadTask = common_vendor.index.uploadFile({
          ...
          fail: (error) => {
            common_vendor.index.__f__("error", "at subPages/order/components/evaluate.vue:434", "上传失败", error);
    ...
    return (_ctx, _cache) => {
      ...
      return common_vendor.e({
        ...
        E: common_vendor.o(toComments)
      } : {}, {
        F: edit.value
      }, edit.value ? {
        G: common_vendor.o(toSubmit)
      } : {}, {
        H: !edit.value && itemData.value.length > 0
      }, !edit.value && itemData.value.length > 0 ? {
        I: common_vendor.f(itemData.value, (item, index, i0) => {
          return {
            a: item.serveItemImg,
        ...
        J: common_vendor.n(itemData.value.length > 0 ? "" : "grey"),
        K: common_vendor.n(edit.value ? "evaluate" : "bg-wt evaluate appList")
      });
    };
  }
};
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["__scopeId", "data-v-f5989750"]]);
wx.createPage(MiniProgramPage);
//# sourceMappingURL=../../../../.sourcemap/mp-weixin/subPages/order/components/evaluate.js.map
