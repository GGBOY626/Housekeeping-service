...
public class ILoginServiceImpl implements ILoginService {
    ...
    @Override
    public LoginResDTO loginForCommonUserByPassword(LoginForCustomerPasswordReqDTO reqDTO) {
        if (StringUtils.isEmpty(reqDTO.getPhone()) || StringUtils.isEmpty(reqDTO.getPassword())) {
            throw new BadRequestException("手机号和密码不能为空");
        }
        CommonUser commonUser = commonUserService.findByPhone(reqDTO.getPhone());
        if (commonUser == null) {
            throw new BadRequestException("账号或密码错误");
        }
        if (CommonStatusConstants.USER_STATUS_FREEZE == commonUser.getStatus()) {
            throw new CommonException(ErrorInfo.Code.ACCOUNT_FREEZED, commonUser.getAccountLockReason());
        }
        if (commonUser.getPassword() == null || !passwordEncoder.matches(reqDTO.getPassword(), commonUser.getPassword())) {
            throw new BadRequestException("账号或密码错误");
        }
        String token = jwtTool.createToken(commonUser.getId(), commonUser.getNickname(), commonUser.getAvatar(), UserType.C_USER);
        return new LoginResDTO(token);
    ...
    @Override
    public void register(RegisterForCustomerReqDTO reqDTO) {
        if (StringUtils.isEmpty(reqDTO.getPhone()) || StringUtils.isEmpty(reqDTO.getPassword())) {
            throw new BadRequestException("手机号和密码不能为空");
        }
        ...
        CommonUser exist = commonUserService.findByPhone(reqDTO.getPhone());
        if (exist != null) {
            throw new BadRequestException("该手机号已注册");
        }
        CommonUser commonUser = new CommonUser();
        commonUser.setId(IdUtil.getSnowflakeNextId());
        commonUser.setPhone(reqDTO.getPhone());
        commonUser.setPassword(passwordEncoder.encode(reqDTO.getPassword()));
        commonUser.setNickname(StringUtils.isNotEmpty(reqDTO.getNickname()) ? reqDTO.getNickname() : "用户" + RandomUtil.randomInt(10000, 99999));
        commonUser.setAvatar("https://yjy-oss-videos.oss-accelerate.aliyuncs.com/tx.png");
        commonUser.setStatus(CommonStatusConstants.USER_STATUS_NORMAL);
        commonUser.setIsDeleted(0);
        commonUserService.save(commonUser);
    }
}
